...
window.createDimScreen = function(api, controls={}) {
  ...
  function show(test=false){
    ...
    hiddenNodes=[...document.body.children].filter(n=>n!==overlay&&n.tagName!=='SCRIPT'&&n.tagName!=='STYLE').map(n=>[n,n.inert,n.getAttribute('aria-hidden')]);
  ...
  function paint(){
    ...
    $('dimMode').textContent=preview?'節電画面 · プレビュー':'節電画面';$('dimNext').textContent=seg?.to||st.next||'確認中';
    ...
    if(routeMode)renderRoute(st);else{
      $('dimFrom').textContent=seg?.from||'現在地を確認中';$('dimPercent').textContent=known?Math.round(ratio*100)+'%':'—';
